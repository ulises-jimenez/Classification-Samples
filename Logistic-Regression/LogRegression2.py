...
train = pd.DataFrame(train, columns = ['Lag1', 'Lag2', 'Intercept'])
...
test = pd.DataFrame(test, columns = ['Lag1', 'Lag2', 'Intercept'])
...
test_up = testresults[testresults == 'Up']
test_down = testresults[testresults == 'Down']
...
test_up_success_rate = float(correct_ups)/float(len(test_up))
test_down_success_rate = float(correct_downs)/float(len(test_down))
